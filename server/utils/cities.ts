...
/** Hard cap on cities per response — bounds the Open-Meteo fan-out and on-screen clutter. */
...
function minPopForZoom(zoom: number): number {
  if (zoom <= 4.5) return Infinity // capitals only
  if (zoom <= 5.5) return 1_000_000
  if (zoom <= 6.5) return 500_000
  if (zoom <= 7.5) return 250_000
  if (zoom <= 8.5) return 100_000
  if (zoom <= 9.5) return 50_000
