...
describe('selectCities', () => {
  it('shows only capitals at low zoom', () => {
    const result = selectCities(EUROPE, 3)
    expect(result.length).toBeGreaterThan(0)
    expect(result.every((c) => c.capital)).toBe(true)
  })

  it('reveals larger cities as zoom increases, respecting the population threshold', () => {
    const capitalsOnly = selectCities(EUROPE, 3)
    const withCities = selectCities(EUROPE, 6)
    // More places appear once we zoom in.
    expect(withCities.length).toBeGreaterThan(capitalsOnly.length)
    // Every non-capital shown at zoom 6 clears the 500k threshold for that tier.
    expect(withCities.filter((c) => !c.capital).every((c) => c.pop >= 500_000)).toBe(true)
  })

  it('always includes capitals even when below the zoom population threshold', () => {
    const result = selectCities(EUROPE, 6) // threshold 500k
    expect(result.some((c) => c.capital && c.pop < 500_000)).toBe(true)
  })
