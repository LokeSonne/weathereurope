# T-Shirt Weather

**T-Shirt Weather** is an interactive map of cities worldwide showing per-day weather
(temperature + condition icon) for a selectable date range. Capitals show first; smaller
cities and towns reveal as you zoom in, and a **t-shirt weather** toggle highlights the
places where it's warm and dry enough for just a tee. Built with Nuxt 4 + MapLibre GL,
weather from [Open-Meteo](https://open-meteo.com/) (best-match model per location), city data from
[GeoNames](https://www.geonames.org/).

## Development

```bash
pnpm install
pnpm dev          # http://localhost:3000
pnpm typecheck
```

## Architecture

- **`app/`** — Nuxt app. `WeatherMap.vue` renders MapLibre + DOM city markers;
  `DayRange.vue` is the from–to date-range picker.
- **`server/api/city-forecast.get.ts`** — given a viewport (`minLng,minLat,maxLng,maxLat,zoom`),
  selects the cities to show and returns them as GeoJSON with an 8-day forecast each.
- **`server/utils/cities.ts`** — zoom→population tiers (capitals always shown), capped at 250 cities.
  Scans a small bundled "prominent" index for wide/mid zoom and loads per-cell chunks on demand
  when zoomed in. See [CACHING.md](CACHING.md#city-data-chunking).
- **`server/api/geo.get.ts`** — coarse IP geolocation (Vercel edge headers) for the initial map
  view; the map opens on the visitor's approximate area, falling back to a world view off-Vercel.
- **`server/utils/openMeteo.ts`** — bulk forecast fetch (batched) with a stale-while-error cache.
- **`server/utils/rateLimit.ts`** — per-IP request limiter.
- **`server/data/`** — generated by `scripts/build-cities.mjs`: `cities-prominent.json` (imported at
  boot), `cities/{cx}_{cy}.json` (on-demand tail chunks), and `cities-manifest.json` (grid config).

### Regenerating the city data

```bash
pnpm build:cities   # re-downloads GeoNames cities5000 (worldwide) and rewrites the chunked city data
```

Requires `curl` and `unzip` on PATH. GeoNames data is CC BY 4.0 and is attributed in the UI.

## Deploying to Vercel

Nuxt auto-detects Vercel — no `vercel.json` or preset needed. Import the repo and deploy.

### Environment variables

| Variable | Purpose | Required |
| --- | --- | --- |
| `UPSTASH_REDIS_REST_URL` | Shared cache + rate-limit store (REST) | Recommended in prod |
| `UPSTASH_REDIS_REST_TOKEN` | Upstash auth token | Recommended in prod |
| `NUXT_PUBLIC_SITE_URL` | Canonical origin for SEO (canonical link, `og:url`, `og:image`, sitemap). Auto-detected on Vercel; set this for a custom domain, e.g. `https://tshirtweather.app` | Custom domain only |
| `NUXT_OPEN_METEO_CONTACT` | Contact string sent in the Open-Meteo `User-Agent` (use a role address) | Recommended |
| `NUXT_OPEN_METEO_API_KEY` | Open-Meteo **commercial** API key (auto-switches to the customer endpoint) | See licensing |
| `NUXT_OPEN_METEO_BASE_URL` | Override the forecast base URL (e.g. a self-hosted instance) | See licensing |

**Why Upstash:** on serverless the default cache is per-instance in-memory — wiped on cold
start and not shared, so hit rates collapse and every viewport re-hits Open-Meteo. Set the
two `UPSTASH_*` vars (any Upstash Redis database, free tier is fine) and both the forecast
cache and the rate limiter become shared and durable. Without them the app still runs, just
with a weaker (per-instance) cache — fine for local dev. To use Vercel KV or another store
instead, swap the driver in `nuxt.config.ts` (`nitro.storage`).

### Weather data licensing (decide before a public launch)

Open-Meteo's free API is **non-commercial only**. The code supports every path via config —
no code change needed:

| Path | Config | When |
| --- | --- | --- |
| **Free non-commercial ← current choice** | (none) | Personal / demo / internal, non-revenue |
| Commercial plan | `NUXT_OPEN_METEO_API_KEY` | Public/commercial site; simplest to run |
| Self-host Open-Meteo | `NUXT_OPEN_METEO_BASE_URL` (point at your instance) | High volume / no per-call cost, you run the infra |

Precedence: explicit base URL → customer endpoint (when a key is set) → free API.

> ⚠️ **This deployment runs on the free, non-commercial tier by design.** Do **not** put it
> behind ads, a paywall, or any revenue-generating/commercial use without first switching to
> the commercial plan or a self-hosted instance (set the env var above — no code change). If
> the intended use ever changes, revisit this decision.

### Notes / limitations

- **Rate limiting** is a best-effort abuse backstop (fixed window, non-atomic), currently
  60 req/min per IP — tune in `city-forecast.get.ts`.
- **Caching:** three stacked layers — browser HTTP cache, Vercel edge (`s-maxage`/
  `stale-while-revalidate`), and a shared per-city forecast cache with stale-on-error fallback.
  The client quantizes its bbox/zoom so nearby/repeat viewports share a URL and hit the HTTP
  caches. See [CACHING.md](CACHING.md) for the full strategy and tuning knobs.
